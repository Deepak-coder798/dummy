...
const Profile = () => {
    ...
    const updateProfileImage = async () => {
        ...
        const response = await axios.put(`http://localhost:5000/uploadProfileImage/${userId}`, { profileImage });
        if (response && response.status == 200) {
            alert(response.data.message);
            setRRR((pre) => !pre);
        ...
        <>
            ...
            {showImageModal && <div style={{
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '20px' }}>
                        <label>Image URL:</label>
                        <input type="text" value={profileImage} onChange={(e) => setProfileImage(e.target.value)} style={{
                            padding: '8px', border: '1px solid #ccc', borderRadius: '4px'
                        }} />
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px' }}>
